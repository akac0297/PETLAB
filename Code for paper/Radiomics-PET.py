...
"""
This program runs radiomics analysis on Before, During, and After PST FDG-PET images
"""
...
def radiomics_analysis(image_filepath, mask_filepath,img_label):
    ...
    #No need to do Z-score normalisation for PET

    #Isotropic voxel resampling - want PET images to be 2 × 2 × 2 mm3
    ...
    #Grey-level discretisation for PET - instead of bin number we should change to bin size (=0.3)
    params={}
    params["binWidth"]=0.3 
...
def getImageLabel(img_fp):
    timepoint=img_fp[-12]
    if timepoint=="1":
        label="Before-PST PET"
    elif timepoint=="2":
        label="During-PST PET"
    elif timepoint=="3":
        label="Post-PST PET"
    return(label)
...
for patient_no in patient_list:
    prePST_PET,durPST_PET, postPST_PET=getImgFilepaths(path,patient_no)
    Bef_Tumour, Dur_Tumour, Post_Tumour=getMaskFilepaths(mask_path,patient_no)

    img_label=getImageLabel(prePST_PET)
    dict1=radiomics_analysis(prePST_PET, Bef_Tumour,img_label)
    dict1.update({'Patient': patient_no})
    df=df.append(dict1,ignore_index=True)

    img_label=getImageLabel(durPST_PET)
    dict2=radiomics_analysis(durPST_PET, Dur_Tumour,img_label)
    dict2.update({'Patient': patient_no})
    df=df.append(dict2,ignore_index=True)

    img_label=getImageLabel(postPST_PET)
    dict3=radiomics_analysis(postPST_PET, Post_Tumour,img_label)
...
print(df)
df.to_csv("/home/alicja/PET-LAB Code/PET-LAB/Radiomics/PET_radiomics_features_Feb_02_22.csv")